...
exports.WolfStickyController = exports.wolfStickyControllerFactory = void 0;
const helpers_1 = require("./helpers");
function wolfStickyControllerFactory(decorationController) {
    return new WolfStickyController(decorationController);
}
exports.wolfStickyControllerFactory = wolfStickyControllerFactory;
class WolfStickyController {
    constructor(decorationController) {
        this.decorationController = decorationController;
        this.updateStickyDecorations = (event, oldLineCount) => {
            const activeEditor = helpers_1.getActiveEditor();
            ...
            if (event.contentChanges.length === 1) {
                ...
                const actualEndLineNo = end.line + 1;
                if (event.document.lineCount != oldLineCount) {
                    if (event.document.lineCount > oldLineCount) {
                        // Added lines
                        if (event.contentChanges[0].text.startsWith("\n")) {
                            if (start.character > 0) {
                                if (helpers_1.annotatedLineIsChanged(activeDocument, start.line, decorations)) {
                                    this.decorationController.deleteDecorationAtLine(start.line + 1);
                                }
                            }
                            if (end.character > 0) {
                                if (helpers_1.annotatedLineIsChanged(activeDocument, end.line, decorations)) {
                        ...
                        if (diff === 1 && text === "") {
                            ...
                            if (start.character === 0 && end.character === 0) {
                                ...
                                this.decorationController.deleteDecorationsAndShiftUp(start.line, start.line, 1);
                            }
                            else {
                                if (start.character > 0) {
                            ...
                            if (start.character === 0) {
                                ...
                                if (end.character === 0) {
                                    ...
                                    this.decorationController.deleteDecorationsAndShiftUp(start.line, end.line);
                                }
                            }
                            else {
                                // edit beginning NOT AT start of a line
                                this.decorationController.deleteDecorationsAndShiftUp(start.line + 1, end.line);
                            }
                        }
                    }
                }
                ...
                if (activeEditor.selections.length === 1) {
                    ...
                    // whats this for?
                    const start = event.contentChanges[1].range.end.line;
                    const end = event.contentChanges[0].range.end.line;
...
exports.WolfStickyController = WolfStickyController;
